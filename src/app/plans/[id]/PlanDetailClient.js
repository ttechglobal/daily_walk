...
// Changes:
//  • Three-dot menu → Leave Plan (members) / Delete Plan (creator only)
//  • "Daily Reflections" button routes to /plans/[id]/reflections
//  • Completing the final day routes to /plans/complete?planId=X
//  • Thread/reflection section removed — lives on the reflections page now
...
import {
  ...
  MessageSquare, MoreVertical, Trash2, LogOut,
} from 'lucide-react'
...
import {
  ...
  sendPlanNudge, joinPlan, notifyReadComplete, leavePlan,
...
export default function PlanDetailClient({ planId }) {
  ...
  async function handleMarkComplete() {
    ...
    try {
      ...
      setMyMembership(prev => prev
        ? { ...prev, currentDay: Math.min(nextDay, total) }
        : prev
      )

      // If final day — go to completion screen
      if (currentDay >= total) {
        setTimeout(() => router.push(`/plans/complete?planId=${planId}`), 600)
        return
      }

      // Otherwise open reflections for the completed day
      router.push(`/plans/${planId}/reflections?day=${currentDay}`)
    } catch (e) {
  ...
  async function handleLeave() {
    ...
    try {
      ...
      showToast('You have left the plan')
      router.push('/plans')
  ...
  async function handleDelete() {
    setMenuOpen(false)
    if (!confirm(`Delete "${plan?.name}"? This will remove the plan for all ${plan?.memberCount || 1} members and cannot be undone.`)) return
    try {
      const sb = createClient()
      if (sb) {
        await sb.from('shared_plans').delete().eq('id', planId)
      }
            ...
            <motion.div key="reading" initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
              ...
              {!isCompleted && isMember && daysBehind > 0 && (
                <BehindCard daysBehind={daysBehind}
                  onCatchUp={() => router.push(`/plans/${planId}?day=${currentDay}`)}
                  ...
                  <button
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      {members.length > 1 && todayReadCount > 0 && (
                        <span className="ml-auto text-[11px] text-white/60">{todayReadCount} read today</span>
                      )}
                    </div>
              ...
              {isMember && completions.size > 0 && (
                <button onClick={() => router.push(`/plans/${planId}/reflections`)}
                  ...
                  style={{ background:t.bgCard, border:`1.5px solid #5B4FCF30`, boxShadow:t.shadow }}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[12px] mt-0.5" style={{ color:t.textMuted }}>
                      See what your group shared — {completions.size} day{completions.size!==1?'s':''} of reflections
                    </p>
                  </div>
            ...
            <motion.div key="group" initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
              ...
              {members.length === 0 ? (
                <div className="text-center py-10">
                  <p className="font-semibold text-[15px]" style={{ color:t.text }}>No members yet</p>
                  <button onClick={shareInvite} className="mt-4 px-5 py-2.5 rounded-full text-white font-bold text-[13px]"
                    style={{ background:'#5B4FCF' }}>Share invite</button>
                </div>
              ) : (
                members.map(member => (
