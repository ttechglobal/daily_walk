...
// 4 items: Home · Bible · Plans · Profile
// Communities and Q&A are hidden (not deleted).
  ...
  return (
    <aside className="hidden md:flex flex-col w-[220px] lg:w-[240px] flex-shrink-0 border-r"
        ...
        {NAV_ITEMS.map(item => {
          ...
          return (
            <Link key={item.href} href={item.href}
              ...
              style={{ background:active?'#EDE9FF':'transparent', color:active?'#5B4FCF':'#6B7280', fontWeight:active?700:500 }}>
              {item.special
                ? <div className="w-7 h-7 rounded-xl flex items-center justify-center" style={{ background:active?'#5B4FCF':'#5B4FCF22' }}><BibleIcon size={14}/></div>
      ...
      <div className="px-5 py-4 border-t" style={{ borderColor:'#F0EDE8' }}>
        <p className="text-[11px] font-semibold" style={{ color:'#C4C1BC' }}>
          Helping believers grow in God's Word — together.
        </p>
      </div>
    </aside>
  )
